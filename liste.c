...
int InsertElementAt(LinkedList *Liste, int i, Enregistrement pers) {
	...
	if (CurrentElement != NULL) {
		...
		if (Liste->size == 0) { // insertion en tête de l'unique élément
			NewElement = NewLinkedListElement(pers);
			if (NewElement != NULL) {
				//Si aucun élément dans la liste, nouvel élément devient queue et tête. Incrémentation taille liste.
				Liste->head = NewElement;
		...
		if (Liste->size <= i) { // insertion en queue
			//Si indice supérieur à taille liste, insertion du maillon à la fin => devient la queue. Incrémentation taille liste.
			NewElement = NewLinkedListElement(pers);
			if (NewElement != NULL) {
...
int DeleteLinkedListElem(LinkedList * list, SingleLinkedListElem * item) {
	...
	SingleLinkedListElem* prior = NULL;
	...
	if ((prior != NULL) && (tmp == item) && (tmp->next == NULL)) {
		list->tail = prior;
		prior->next = NULL;
		list->size--;
		free(item);
